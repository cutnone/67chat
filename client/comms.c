...
void disconnectFromServer() {
    #ifdef _WIN32
        closesocket(sock);   // not close() like on Linux
        WSACleanup();
    #else
        close(sock);
    #endif
...
//sample function to receive server messages
void receiveLoop() {
    ...
    while (1) {
        ...
        if (n <= 0) break;
        buf[n] = 0;
        printf("%s", buf); // i mean idk what else to do with it so ig just print the messages for now LMAO
    }
}